...
export async function recordPaymentWithCoverage(input: unknown) {
  ...
  const payment = await tenantPrisma.payment.create({ data: { id: crypto.randomUUID(), studentId: values.studentId, amount: values.amount, date: values.date, method: values.method, status: values.status, billingPeriod: values.billingPeriod, coveredMonth: values.billingPeriod === "MONTHLY" ? values.coveredMonth : null, coveredFromDate: values.billingPeriod === "CLASSWISE" ? values.coveredFromDate : null, coveredToDate: values.billingPeriod === "CLASSWISE" ? values.coveredToDate : null, coveredClassCount: values.billingPeriod === "CLASSWISE" ? values.coveredClassCount : null, notes: values.notes } });
