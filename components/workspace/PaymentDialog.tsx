...
export default function PaymentDialog({ open, studentName, onOpenChange, onSubmit, defaultBillingPeriod = BillingPeriod.MONTHLY }: Readonly<PaymentDialogProps>) {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (!coveredMonth) setCoveredMonth(getTodayDateKey().slice(0, 7));
    }
  }, [open, defaultBillingPeriod, coveredMonth]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100%-1rem)] border-border-strong bg-surface text-foreground sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Record a payment from {studentName} and specify exactly what the payment covers.</DialogDescription>
        </DialogHeader>
        <form className="grid max-h-[min(78dvh,720px)] gap-4 overflow-y-auto pr-1" onSubmit={(event) => void submit(event)}>
          ...
          {billingPeriod === BillingPeriod.MONTHLY ? (
            <div className="rounded-2xl border border-primary/15 bg-primary/5 p-4">
              <label htmlFor="payment-covered-month" className="grid gap-1.5 text-sm font-medium">
                Month covered by this payment
                <Input id="payment-covered-month" required type="month" value={coveredMonth} onChange={(event) => setCoveredMonth(event.target.value)} />
              </label>
              <p className="mt-1.5 text-xs text-muted-foreground">The payment date is when money was received; this month is the tuition period it settles.</p>
            </div>
          ) : (
            <div className="rounded-2xl border border-primary/15 bg-primary/5 p-4">
              <p className="text-sm font-medium">Classes covered by this payment</p>
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <label htmlFor="payment-from" className="grid gap-1.5 text-xs font-medium">From<Input id="payment-from" required type="date" value={coveredFromDate} onChange={(event) => setCoveredFromDate(event.target.value)} /></label>
                <label htmlFor="payment-to" className="grid gap-1.5 text-xs font-medium">To<Input id="payment-to" required type="date" value={coveredToDate} onChange={(event) => setCoveredToDate(event.target.value)} /></label>
                <label htmlFor="payment-class-count" className="grid gap-1.5 text-xs font-medium">Number of classes<Input id="payment-class-count" min={1} required type="number" value={coveredClassCount} onChange={(event) => setCoveredClassCount(event.target.value)} /></label>
              </div>
              <p className="mt-2 text-xs text-muted-foreground">The exact attended sessions can be allocated separately; these fields record the agreed coverage window and class count.</p>
            </div>
          )}
